...
void selectnum()
{
	...
	if (!(choice == 1 || choice == 2 || choice == 3 || choice == 4 || choice == 5 || choice == 6 || choice == 7 || choice == 8 || choice == 9)) //防止用户不输入整数
	{
		printf("Error! Please re-input the number!\n");
		choice = 0;
		selectnum();        //如果输入其他的东西  则重新载入函数
	}
